...
void on_response(int vmajor, int vminor, int code, x0::BufferRef&& text) {
  std::clog << "S< HTTP/" << vmajor << "." << vminor << " " << code << ' '
            << text.str() << std::endl;
}
...
int main(int argc, const char* argv[]) {
  ...
  if (client.state() == x0::WebClient::DISCONNECTED) {
    std::clog << "Could not connect to server: " << client.lastError().message()
              << std::endl;
    return -1;
  }
  ...
  for (int i = 0; i < request_count; ++i) {
    ...
    client.commit(i == (request_count - 1));  // pass true on last iteration,
                                              // false otherwise.
  }
