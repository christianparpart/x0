...
bool response::header_contains(const std::string& key,
...
response request(
  ...
  tcp::resolver::query query(hostname_,
  ...
  if (response.size() > 0) content << &response;
...
std::pair<std::string, std::string> header(const std::string& key,
                                           const std::string& value) {
  return std::make_pair(key, value);
}

class http11 : public CPPUNIT_NS::TestFixture {
 public:
 ...
 private:
  void _404() {
    auto response = request("GET /404 HTTP/1.0",
                            {header("foo", "bar"), header("com", "tar")});
  ...
  void range1() {
    auto response =
        request("GET /12345.txt HTTP/1.1", {header("Range", "bytes=0-3")});
  ...
  void range2() {
    auto response =
        request("GET /12345.txt HTTP/1.1", {header("Range", "bytes=1-1")});
  ...
  void range3() {
    auto response =
        request("GET /12345.txt HTTP/1.1", {header("Range", "bytes=0-4")});
  ...
  void range4() {
    auto response = request("GET /12345.txt HTTP/1.1",
                            {header("Range", "bytes=2-2,1-1,0-0")});

    CPPUNIT_ASSERT(response.status == 206);
    CPPUNIT_ASSERT(
        response.header_contains("Content-Type", "multipart/byteranges"));
    CPPUNIT_ASSERT(response.content.find("Content-Type: text/plain") !=
                   std::string::npos);
  }
  // }}}
};
