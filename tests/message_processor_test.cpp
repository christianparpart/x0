...
#warning \
    "Compiler does not support lambda expressions. Cannot unit-test HttpMessageParser"
#else
...
class HttpMessageParser_component :  // {{{
 ...
 private:
  virtual bool onMessageBegin(const BufferRef& method, const BufferRef& uri,
                              int version_major, int version_minor) {
    if (on_request) on_request(method, uri, version_major, version_minor);

    return true;
  }

  virtual bool onMessageBegin(int version_major, int version_minor, int code,
                              const BufferRef& text) {
    if (on_status) on_status(version_major, version_minor, code, text);
  ...
  virtual bool onMessageHeader(const BufferRef& name, const BufferRef& value) {
    if (on_header) on_header(name, value);

    return true;
  }

  virtual bool onMessageHeaderEnd() {
    if (on_header_done) return on_header_done();

    return true;
  }

  virtual bool onMessageContent(const BufferRef& chunk) {
    if (on_content) return on_content(chunk);
...
class HttpMessageParser_test : public CPPUNIT_NS::TestFixture {
 public:
 ...
 private:
  ...
  void request_simple() {
    HttpMessageParser_component rp(
        HttpMessageParser::REQUEST);  // (message_processor::REQUEST);

    rp.on_request = [&](const BufferRef& method, const BufferRef& entity,
                        int major, int minor) {
  ...
  void response_simple() {
    ...
    rp.on_status = [&](int vmajor, int vminor, int code,
                       const BufferRef& text) {
  ...
  void request_no_header() {
    ...
    int request_count = 0;
    rp.on_request = [&](const BufferRef& method, const BufferRef& url,
                        int major, int minor) {
 ...
 private:  // message tests
  void message_chunked_body() {
    ...
    rp.on_complete = [&]()->bool { return false; };
