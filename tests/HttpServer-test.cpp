...
class HttpServerTest : public ::testing::Test {
 public:
  ...
  void request(
      ...
      const Buffer& content, HttpClient::ResponseHandler callback);
...
void HttpServerTest::SetUp() { http_ = new HttpServer(loop_); }
...
TEST_F(HttpServerTest, DISABLED_Get) {
  ...
  HttpClient::request(
      IPAddress("127.0.0.1"), 8080, "GET", "/",
      {{"Foo", "bar"}, {"User-Agent", "HttpClient/1.0"}}, body,
      [&](HttpClientError ec, int status, const HttpClient::HeaderMap& headers,
          const BufferRef& content) {
