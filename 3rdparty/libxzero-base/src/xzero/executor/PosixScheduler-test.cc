...
TEST(PosixScheduler, executeOnReadable_timeout) {
  ...
  sched.executeOnReadable(pipe.readerFd(), onFire, TimeSpan::fromMilliseconds(500), onTimeout);
  sched.runLoopOnce();
...
TEST(PosixScheduler, executeOnReadable_timeout_on_cancelled) {
  ...
  auto handle = sched.executeOnReadable(
      pipe.readerFd(), onFire, TimeSpan::fromMilliseconds(500), onTimeout);
...
TEST(PosixScheduler, executeOnWritable) {
  ...
  int fireCount = 0;
  int timeoutCount = 0;
  const TimeSpan timeout = TimeSpan::fromSeconds(1);
