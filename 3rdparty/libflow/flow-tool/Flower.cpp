...
Flower::Flower()
  ...
  registerFunction("__print", FlowType::Void).params(FlowType::String).bind(
      &Flower::flow_print);

  registerFunction("__print", FlowType::Void).params(FlowType::Number).bind(
      &Flower::flow_print_I);
  ...
  registerFunction("__print", FlowType::Void).params(FlowType::IntArray).bind(
      &Flower::flow_print_i);
  ...
  registerFunction("__print", FlowType::Void).params(FlowType::CidrArray).bind(
      &Flower::flow_print_c);

  registerFunction("suspend", FlowType::Void).bind(&Flower::flow_suspend);
  ...
  registerHandler("error").param<FlowString>("message", "").bind(
      &Flower::flow_error);
...
bool Flower::onParseComplete(Unit* unit) {
  ...
  for (auto& call : callv.calls()) {
    ...
    if (StringExpr* descriptionExpr =
            dynamic_cast<StringExpr*>(args.values()[1])) {
...
int Flower::runAll(const char* fileName) {
  ...
  if (dumpAST_) ASTPrinter::print(unit.get());
