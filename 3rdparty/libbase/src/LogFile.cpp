...
void LogFile::cycle() { onCycle_.send(); }
...
void LogFile::accept(SinkVisitor& v) { v.visit(*this); }
...
void LogFile::onData(ev::io& io, int revents) {
  ...
  // move possible valid tail data to the beginning of the read-buffer, and cut
  // the rest
...
retry:
  ssize_t rv = ::read(receiverFd_, readBuffer_.end(),
