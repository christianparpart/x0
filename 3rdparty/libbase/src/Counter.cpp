...
Counter::Counter() {}
#else
Counter::Counter() : current_(0), max_(0), total_(0) {}
#endif
...
bool Counter::increment(size_t n, size_t expected) {
  ...
  if (!current_.compare_exchange_weak(expected, desired)) return false;

  // XXX this might *not always* result into the highest value, but we're fine
  // with it.
  if (desired > max_.load()) max_.store(expected + n);
...
void Counter::increment(size_t n) {
  ...
  if (current_ > max_) max_.store(current_.load());
