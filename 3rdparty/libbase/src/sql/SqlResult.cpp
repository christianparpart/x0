...
bool SqlResult::isError() const { return !errorText_.empty(); }

unsigned SqlResult::currentRow() const { return currentRow_; }

unsigned SqlResult::numRows() const { return numRows_; }

unsigned SqlResult::numFields() const { return mysql_num_fields(result_); }
...
const char* SqlResult::nameAt(int index) const { return fields_[index]->name; }

const std::string& SqlResult::valueOf(const char* name) const {
  for (int i = 0, e = fields_.size(); i != e; ++i)
    if (strcmp(fields_[i]->name, name) == 0) return data_[i];

  static const std::string none;
  return none;
}

const std::string& SqlResult::valueAt(int index) const { return data_[index]; }
