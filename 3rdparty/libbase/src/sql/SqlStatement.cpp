...
#include <base/sql/SqlStatement.h>
...
std::string SqlStatement::valueAt<std::string>(unsigned index) const {
  if (nulls_[index]) return std::string();
  ...
  switch (fields_[index]->type) {
    ...
    case MYSQL_TYPE_DATE: {
      ...
      char buf[11];
      snprintf(buf, sizeof(buf), "%04d-%02d-%02d", ts->year, ts->month,
               ts->day);
      return buf;
    }
    case MYSQL_TYPE_TIME: {
      MYSQL_TIME *ts = (MYSQL_TIME *)d->buffer;
      char buf[9];
      snprintf(buf, sizeof(buf), "%02d:%02d:%02d", ts->hour, ts->minute,
               ts->second);
      return buf;
    }
...
bool SqlStatement::isError() const { return error_ != NULL; }

const char *SqlStatement::error() const { return error_; }
...
bool SqlStatement::fetch() {
  ...
  for (unsigned i = 0, e = data_.size(); i != e; ++i) {
    ...
    if (fixedLengths_[i]) {
      // TRACE("fixed-length value[%d]: %s", i,
      // this->valueAt<std::string>(i).c_str());
      continue;
    }
...
bool SqlStatement::isNullAt(unsigned index) const { return nulls_[index]; }

unsigned SqlStatement::numFields() const { return fields_.size(); }

unsigned SqlStatement::currentRow() const { return currentRow_; }
...
template <>
bool SqlStatement::bindParam<unsigned long long>(
...
char SqlStatement::valueAt<char>(unsigned index) const {
  ...
  switch (fields_[index]->type) {
    ...
    case MYSQL_TYPE_VAR_STRING:
    case MYSQL_TYPE_VARCHAR:
      return d->buffer_length ? static_cast<char *>(d->buffer)[0] : 0;
    case MYSQL_TYPE_TINY:
...
long SqlStatement::valueAt<long>(unsigned index) const {
  ...
  switch (fields_[index]->type) {
    ...
    case MYSQL_TYPE_DATETIME: {
      ...
      // XXX is this right? however, this seems to be the way to get the right
      // timestamp in my case at least (as of 2011-04-30 CEST)
      if (tm.tm_isdst > 0) ts += 3600;
...
template <>
unsigned long long SqlStatement::valueAt<unsigned long long>(
