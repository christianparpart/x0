 ...
 *
 * @param filename path to the fielsystem's file.  The filename \b /dev/stdout
 *and \b /dev/stderr
 *                 are handled specially, not directly opening that file, but
 *assigning
 *                 its well known file descriptors (1 and 2) directly.
 *                 These file descriptors, unlike any other, are not closed on
 *destruction.
 * @param flags    Flags, passed to the open(2) system call, e.g. O_WRONLY.
 * @param mode     Access mode used upon creation of this file.
 *
 * If possible, the FD_CLOEXEC flag is enabled after a successful open(2), too,
 *but
 * if you rely on this behavior and are in a multi threaded environment with
 * fork()'s in between, it is generally recommended to pass \b O_CLOEXEC as \a
 *flag aswell.
 */
 ...
 * @param autoClose whether or not the given file descriptor shall be closed
 *upon instance destruction.
 *
 */
FileSink::FileSink(int fd, bool autoClose)
    : path_(), flags_(0), mode_(0), handle_(fd), autoClose_(autoClose) {}
...
void FileSink::accept(SinkVisitor& v) { v.visit(*this); }
