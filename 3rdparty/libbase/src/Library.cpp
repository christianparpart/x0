...
// {{{ errors
enum class dlfcn_error { success = 0, generic_error = 1 };

class dlfcn_error_category_impl : public std::error_category {
 private:
  std::vector<std::string> vector_;

#ifndef __APPLE__
 public:
  dlfcn_error_category_impl() { vector_.push_back("Success"); }
#endif
  ...
  std::error_code make() {
    if (const char* p = dlerror()) {
      ...
      for (int i = 0, e = vector_.size(); i != e; ++i)
        if (vector_[i] == msg) return std::error_code(i, *this);
  ...
  virtual const char* name() const noexcept(true) { return "dlfcn"; }

  virtual std::string message(int ec) const { return vector_[ec]; }
};
...
template <>
struct is_error_code_enum<dlfcn_error> : public true_type {};
}
...
Library::Library(const std::string& filename) : handle_(0) {
...
Library::Library(Library&& movable)
    : filename_(std::move(movable.filename_)), handle_(movable.handle_) {
  movable.handle_ = 0;
}
...
bool Library::isOpen() const { return handle_ != 0; }
