...
class BASE_API Socket
 ...
 public:
  ...
  enum State { Closed, Connecting, Handshake, Operational };
 ...
 public:
  ...
  void setTimeout(K *object, TimeSpan value);
  ...
  void setReadyCallback(K *object);
  void clearReadyCallback();

  // initiates the handshake
  template <class K, void (K::*cb)(Socket *)>
  void handshake(K *object);
 ...
 private:
  static State open(struct ev_loop *loop, const SocketSpec &spec, int flags,
                    int *fd);
...
// {{{ inlines
inline int Socket::handle() const { return fd_; }
...
inline Socket::State Socket::state() const { return state_; }

inline void Socket::setState(State s) { state_ = s; }

inline Socket::Mode Socket::mode() const { return mode_; }

inline bool Socket::tcpCork() const { return tcpCork_; }

inline bool Socket::splicing() const { return splicing_; }

inline void Socket::setSplicing(bool enable) { splicing_ = enable; }

inline bool Socket::isOpen() const { return fd_ >= 0; }

inline bool Socket::isClosed() const { return fd_ < 0; }

inline bool Socket::isSecure() const { return secure_; }

inline void Socket::setSecure(bool enabled) { secure_ = enabled; }
...
#if !defined(NDEBUG)
  debug("setTimeout(%p, %.2f) active:%s", object, value(),
        timer_.is_active() ? "true" : "false");
#endif
...
inline void Socket::callback(int revents) {
  if (callback_) callback_(this, callbackData_, revents);
}

inline struct ev_loop *Socket::loop() const { return loop_; }
