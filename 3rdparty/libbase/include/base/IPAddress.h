...
namespace std {
template <>
struct hash<base::IPAddress> : public unary_function<base::IPAddress, size_t> {
  size_t operator()(const base::IPAddress& v) const {
    return *(uint32_t*)(v.data());
  }
};

inline std::ostream& operator<<(std::ostream& os, const base::IPAddress& ipaddr) {
  os << ipaddr.str();
  return os;
}
}
