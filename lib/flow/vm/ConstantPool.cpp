...
// {{{ helpers
template<typename T, typename S> std::vector<T>&& convert(const std::vector<S>& source)
{
...
inline size_t ensureValue(std::vector<std::vector<T>>& vv, const U& array)
{
    ...
    // it might be, that the source element type is not the same as the target element type
...
ConstantPool& ConstantPool::operator=(ConstantPool&& v)
{
    return *this;
}
