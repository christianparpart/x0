...
x0_inifile_t* x0_inifile_create() { return new x0_inifile_t(); }

void x0_inifile_destroy(x0_inifile_t* ini) { delete ini; }
...
void x0_inifile_clear(x0_inifile_t* ini) { ini->obj.clear(); }

size_t x0_inifile_section_count(x0_inifile_t* ini) { return ini->obj.size(); }
...
ssize_t x0_inifile_value_get(x0_inifile_t* ini, const char* title,
  ...
  std::string result;
  if (!ini->obj.get(title, key, result)) return 0;
