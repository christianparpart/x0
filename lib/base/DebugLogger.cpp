...
#define ITRACE(msg...)                         \
  do {                                         \
    printf("DebugLogger(%s): ", tag_.c_str()); \
    printf(msg);                               \
    printf("\n");                              \
  } while (0)
#else
...
DebugLogger::Instance::~Instance() { ITRACE("despawn"); }
...
void DebugLogger::Instance::setPreference(const std::string& value) {
  ...
  if (post_.empty()) post_ = "\033[0m";
}

void DebugLogger::Instance::log(int level, const char* fmt, ...) {
  ITRACE("log(level=%d); verbosity=%d, enabled=%d", level, verbosity_,
         enabled_);
  if (!enabled_) return;
  ...
  if (logger_->colored()) {
    Buffer buf;
    buf.printf("%s[%s:%d] %s%s", pre_.c_str(), tag_.c_str(), level, msg,
               post_.c_str());
...
void logWriteDefault(const char* msg, size_t n) { printf("%s\n", msg); }

DebugLogger::DebugLogger()
    : onLogWrite(std::bind(logWriteDefault, std::placeholders::_1,
                           std::placeholders::_2)),
      configured_(false),
      map_(),
      colored_(true) {}

DebugLogger::~DebugLogger() { reset(); }
...
void DebugLogger::reset() {
  configured_ = false;
  onLogWrite =
      std::bind(logWriteDefault, std::placeholders::_1, std::placeholders::_2),
...
void DebugLogger::enable(const std::string& tag) { get(tag)->enable(); }

void DebugLogger::disable(const std::string& tag) { get(tag)->disable(); }
