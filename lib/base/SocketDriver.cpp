...
bool SocketDriver::isSecure() const { return false; }

Socket *SocketDriver::create(struct ev_loop *loop, int handle, int af) {
  return new Socket(loop, handle, af);
}

Socket *SocketDriver::create(struct ev_loop *loop, IPAddress *ipaddr,
                             int port) {
...
void SocketDriver::destroy(Socket *socket) { delete socket; }
