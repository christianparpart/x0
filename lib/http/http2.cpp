...
{
  static const std::unordered_map<unsigned, std::map<unsigned, std::string>> mapping = {
...
void Frame::dumpHeader() const {
  printf(" * %s: flags=[%s], stream_id=%u, length=%zu\n",
    ...
    to_string(flags(), type()).c_str(),
    streamID(),
    payloadLength()
  );
}

void Frame::setHeader(size_t payloadLength, FrameType type, unsigned flags, unsigned streamID)
{
  reserved1_ = 0;
...
void Frame::encodeHeader(Buffer* out, size_t payloadLength, FrameType type, unsigned flags, unsigned streamID)
{
  Frame frame;
...
void DataFrame::encode(Buffer* out, bool endStream, bool endSegment, unsigned streamID, const BufferRef& data)
{
  unsigned flags = 0;
  ...
  encodeHeader(out, sizeof(pre) + data.size(), FrameType::DATA, flags, streamID);
  out->push_back(&pre, sizeof(pre));
  out->push_back(data);
}

void DataFrame::encode(Buffer* out, bool endStream, bool endSegment,
  unsigned streamID, const BufferRef& data, const BufferRef& padding)
{
  ...
  encodeHeader(out, sizeof(pre) + data.size() + padding.size(), FrameType::DATA, flags, streamID);
  out->push_back(&pre, sizeof(pre));
  out->push_back(data);
...
void PriorityFrame::encode(Buffer* out,
    bool exclusive, unsigned dependantStreamID, uint8_t weight,
    unsigned streamID)
{
  PriorityFrame frame;

  frame.setHeader(sizeof(frame) - sizeof(Frame), FrameType::PRIORITY, 0, streamID);
...
void ResetStreamFrame::encode(Buffer* out, ErrorCode ec, unsigned streamID)
{
  ...
  frame.setHeader(sizeof(frame) - sizeof(Frame), FrameType::RST_STREAM, 0, streamID);
...
SettingsFrame::Parameter SettingsFrame::Parameter::encode(Type type, uint32_t value)
{
  Parameter param;
...
void SettingsFrame::encode(Buffer* out, const std::vector<std::pair<Parameter::Type, uint32_t>>& params)
{
  encodeHeader(out, params.size() * sizeof(Parameter), FrameType::SETTINGS, 0, 0);
...
{
  dumpHeader();
  printf("    ; lastStreamID=%u, errorCode=%s\n", lastStreamID(), to_string(errorCode()).c_str());
...
void GoAwayFrame::encode(Buffer* out, unsigned lastStreamID, ErrorCode ec, const BufferRef& debugData)
{
  GoAwayFrame frame;

  frame.setHeader(sizeof(frame) - sizeof(Frame) + debugData.size(), FrameType::GOAWAY, 0, 0);
  frame.reserved_ = 0;
  frame.lastStreamID_ = htonl(lastStreamID);
  frame.errorCode_ = htonl(static_cast<unsigned>(ec));
...
void WindowUpdateFrame::encode(Buffer* out, unsigned windowSizeIncrement)
{
  ...
  frame.setHeader(sizeof(frame) - sizeof(Frame), FrameType::WINDOW_UPDATE, 0, 0);
  frame.reserved_ = 0;
