...
void HttpClient::setRequest(const std::string& method, const std::string& path, const HeaderMap& headers, const Buffer& content)
{
	...
	if (!content.empty()) {
		writeBuffer_.push_back("Content-Length: " "\r\n");
...
void HttpClient::start()
{
	...
	socket_->open(SocketSpec::fromInet(IPAddress(ipaddr_), port_), O_CLOEXEC | O_NONBLOCK);
	if (!socket_->isOpen()) {
...
bool HttpClient::onMessageBegin(int versionMajor, int versionMinor, int code, const BufferRef& text)
{
	statusCode_ = code;
	statusText_ = text;
	return true;
...
bool HttpClient::onMessageHeader(const BufferRef& name, const BufferRef& value)
{
	headers_[name.str()] = value.str();
	return true;
}

bool HttpClient::onMessageHeaderEnd()
{
	return true;
}
...
void HttpClient::request(const IPAddress& host, int port,
