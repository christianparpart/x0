...
void DirectorPlugin::director_pseudonym(flow::Params& args) {
  pseudonym_ = args.getString(1).str();
}

// {{{ setup_function director.load(name, path)
void DirectorPlugin::director_load(Params& args) {
...
#if defined(ENABLE_DIRECTOR_CACHE)
void DirectorPlugin::director_cache_key(HttpRequest* r, Params& args) {
...
#if defined(ENABLE_DIRECTOR_CACHE)
void DirectorPlugin::director_cache_ttl(HttpRequest* r, Params& args) {
  auto notes = requestNotes(r);
...
bool DirectorPlugin::director_balance(HttpRequest* r, Params& args) {
...
bool DirectorPlugin::director_pass(HttpRequest* r, Params& args) {
  std::string directorName = args.getString(1).str();
...
bool DirectorPlugin::director_api(HttpRequest* r, Params& args) {
...
// "close");
bool DirectorPlugin::director_fcgi(HttpRequest* r, Params& args) {
...
bool DirectorPlugin::director_http(HttpRequest* r, Params& args) {
  SocketSpec socketSpec(args.getIPAddress(1),  // bind addr
                        args.getInt(2)         // port
                        );
