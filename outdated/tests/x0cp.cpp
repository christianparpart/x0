...
int main(int argc, char* argv[]) {
  ...
  x0::Source* input(ifname == "-" ? new x0::FileSource("/dev/stdin")
                                  : new x0::FileSource(ifname.c_str()));

  x0::Sink* output(
      ofname == "-" ? new x0::FileSink("/dev/stdout", O_WRONLY)
                    : new x0::FileSink(ofname, O_WRONLY | O_CREAT | O_TRUNC));

  if (!cf.empty()) input = new x0::FilterSource(input, &cf, true);

  ssize_t rv;
  size_t nwritten = 0;
  while ((rv = input->sendto(*output)) > 0) nwritten += rv;

  if (rv < 0) perror("copy error");
