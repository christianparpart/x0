...
public:
	x0::BufferRef method;
	x0::BufferRef path;
	std::list<std::pair<x0::BufferRef, x0::BufferRef>> headers;
...
private:
	...
	virtual bool onMessageBegin(const x0::BufferRef& method, const x0::BufferRef& entity, int versionMajor, int versionMinor)
	{
		this->method = method;
		path = entity;
...
void FastCgiHealthMonitor::setRequest(const char* fmt, ...)
{
	...
	do {
		...
		va_start(va, fmt);
		blen = vsnprintf(const_cast<char*>(request.data()), request.capacity(), fmt, va);
		va_end(va);
...
bool FastCgiHealthMonitor::writeSome()
{
	...
	ssize_t writeCount = socket_.write(writeBuffer_.data() + writeOffset_, chunkSize);
	...
	for (;;) {
		...
		if (rv < 0) {
			if (errno != EINTR && errno != EAGAIN) { // TODO handle EWOULDBLOCK
				worker_.log(x0::Severity::error, "fastcgi: read from backend %s failed: %s", backend_->socketSpec().str().c_str(), strerror(errno));
				logFailure();
				return false;
			}
...
bool FastCgiHealthMonitor::processRecord(const FastCgi::Record *record)
{
	...
	switch (record->type()) {
		...
		default:
			worker_.log(x0::Severity::error,
				"fastcgi: unknown transport record received from backend %s. type:%d, payload-size:%ld",
				backend_->socketSpec().str().c_str(), record->type(), record->contentLength());
#if 1
...
void FastCgiHealthMonitor::onEndRequest(int appStatus, FastCgi::ProtocolStatus protocolStatus)
{
	...
	// in order to tell us their response status code, so we default to 200 (Ok) here, if
	// and only if the application's status code is 0 (which usually means Success, too).

	if (responseCode_ == x0::HttpStatus::Undefined
			&& protocolStatus == FastCgi::ProtocolStatus::RequestComplete
			&& appStatus == 0) {
		responseCode_ = x0::HttpStatus::Ok;
	}
