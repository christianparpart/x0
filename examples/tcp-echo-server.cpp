...
EchoServer::EchoServer(struct ev_loop* loop, const char* bind, int port, Logger* logger) :
	loop_(loop),
	ss_(nullptr),
	logger_(logger)
{
...
template<typename... Args>
void EchoServer::Session::log(Severity severity, const char* fmt, Args... args)
{
	LogMessage msg(severity, fmt, args...);
...
int main()
{
	...
	std::unique_ptr<EchoServer> es(new EchoServer(loop, "0.0.0.0", 7979, logger.get()));
	ev_run(loop);
	return 0;
}
