...
class X0_API XzeroDaemon : public x0::FlowVM::Runtime {
 private:
 ...
 public:  // FlowBackend overrides
  virtual bool import(const std::string& name, const std::string& path,
                      std::vector<x0::FlowVM::NativeCallback*>* builtins);

  // new
  template <typename... ArgTypes>
  x0::FlowVM::NativeCallback& setupFunction(
      const std::string& name, const x0::FlowVM::NativeCallback::Functor& cb,
      ArgTypes... argTypes);
  template <typename... ArgTypes>
 ...
 private:
  ...
  bool validate(const std::string& context,
                const std::vector<x0::CallExpr*>& calls,
...
template <typename... ArgTypes>
inline x0::FlowVM::NativeCallback& XzeroDaemon::setupFunction(
    const std::string& name, const x0::FlowVM::NativeCallback::Functor& cb,
    ArgTypes... argTypes) {
  setupApi_.push_back(name);
  return registerFunction(name, x0::FlowType::Void).bind(cb).params(
      argTypes...);
}

template <typename... ArgTypes>
inline x0::FlowVM::NativeCallback& XzeroDaemon::sharedFunction(
    const std::string& name, const x0::FlowVM::NativeCallback::Functor& cb,
    ArgTypes... argTypes) {
  setupApi_.push_back(name);
...
template <typename... ArgTypes>
inline x0::FlowVM::NativeCallback& XzeroDaemon::mainFunction(
    const std::string& name, const x0::FlowVM::NativeCallback::Functor& cb,
    ArgTypes... argTypes) {
  mainApi_.push_back(name);
  return registerFunction(name, x0::FlowType::Void).bind(cb).params(
...
template <typename... ArgTypes>
inline x0::FlowVM::NativeCallback& XzeroDaemon::mainHandler(
    const std::string& name, const x0::FlowVM::NativeCallback::Functor& cb,
    ArgTypes... argTypes) {
  mainApi_.push_back(name);
