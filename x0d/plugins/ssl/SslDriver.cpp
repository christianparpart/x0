...
inline SslCacheItem::SslCacheItem() { clear(); }
...
inline void SslCacheItem::set(const gnutls_datum_t& key,
...
inline bool SslCacheItem::equals(const gnutls_datum_t& key) const {
  return key.size == key_size_ && std::memcmp(key.data, key_, key.size) == 0;
}
// }}}
...
bool SslDriver::isSecure() const { return true; }
...
void SslDriver::setPriorities(const std::string& value) {
  ...
  if (rv != GNUTLS_E_SUCCESS) {
    TRACE("gnutls_priority_init: error: %s \"%s\"", gnutls_strerror(rv),
          errp ? errp : "");
  }
}
...
void SslDriver::destroy(x0::Socket* socket) { delete socket; }
...
int SslDriver::_store(void* dbf, gnutls_datum_t key, gnutls_datum_t value) {
  if (reinterpret_cast<SslDriver*>(dbf)->store(key, value)) return 0;
...
int SslDriver::_remove(void* dbf, gnutls_datum_t key) {
  if (reinterpret_cast<SslDriver*>(dbf)->remove(key)) return 0;
