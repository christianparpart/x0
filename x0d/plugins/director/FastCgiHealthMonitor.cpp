...
// {{{ HttpRequestRec
class HttpRequestRec : public x0::HttpMessageParser {
 public:
  x0::BufferRef method;
  x0::BufferRef path;
 ...
 private:
  ...
  virtual bool onMessageBegin(const x0::BufferRef& method,
                              const x0::BufferRef& entity, int versionMajor,
                              int versionMinor) {
    this->method = method;
    path = entity;
  ...
  virtual bool onMessageHeader(const x0::BufferRef& name,
...
void FastCgiHealthMonitor::onStdErr(const x0::BufferRef& chunk) {
  worker_.log(x0::Severity::error, "fastcgi: Health check error. %s",
