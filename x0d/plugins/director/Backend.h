...
class Backend
 ...
 public:
  ...
  void setJsonWriteCallback(
      const std::function<void(const Backend*, x0::JsonWriter&)>& callback);
  ...
  const x0::Counter& load() const {
    return load_;
  }  //!< number of currently being processed requests.

  const x0::SocketSpec& socketSpec() const {
    return socketSpec_;
...
X0_API inline x0::JsonWriter& operator<<(x0::JsonWriter& json,
