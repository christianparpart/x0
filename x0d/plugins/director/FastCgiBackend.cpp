...
FastCgiBackend::Connection::Connection(
    RequestNotes* rn, std::unique_ptr<x0::Socket>&& backendSocket)
...
void FastCgiBackend::Connection::initialize() {
  ...
  if (true) {
    ...
    if (transferHandle_ < 0) {
      snprintf(transferPath_, sizeof(transferPath_),
               X0_TMPDIR "/x0d-director-%d", socket_->handle());
...
void FastCgiBackend::Connection::onConnectComplete(x0::Socket* s, int revents) {
  if (s->isClosed()) {
    log(x0::Severity::error, "Connecting to backend server failed. %s",
        strerror(errno));
...
void FastCgiBackend::Connection::onReadWriteTimeout(x0::Socket* s) {
  log(x0::Severity::error, "I/O timeout to backend %s: %s",
    ...
    for (;;) {
      ...
      if (rv == 0) {
        if (isAborted_) {
          ...
          log(x0::Severity::error, "Reading from backend %s failed: %s.",
              backendName().c_str(), strerror(errno));
          exitFailure(HttpStatus::ServiceUnavailable);
        }
        return;
      ...
      if (rv < 0) {
        if (errno != EINTR && errno != EAGAIN) {  // TODO handle EWOULDBLOCK
          log(x0::Severity::error, "Read from backend %s failed: %s",
              backendName().c_str(), strerror(errno));
          exitFailure(HttpStatus::ServiceUnavailable);
          return;
        }
  ...
  if (revents & x0::Socket::Write) {
    ...
    if (rv < 0) {
      if (errno != EINTR && errno != EAGAIN) {
        log(x0::Severity::error, "Writing to backend %s failed: %s",
            backendName().c_str(), strerror(errno));
        exitFailure(HttpStatus::ServiceUnavailable);
      }
...
bool FastCgiBackend::Connection::onMessageHeader(const x0::BufferRef& name,
...
bool FastCgiBackend::process(RequestNotes* rn) {
  ...
  std::unique_ptr<x0::Socket> socket(
      x0::Socket::open(rn->request->connection.worker().loop(), socketSpec_,
  ...
  if (socket) {
    ...
    rn->request->log(x0::Severity::notice,
