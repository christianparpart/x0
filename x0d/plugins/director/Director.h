...
class Director : public BackendManager {
 ...
 public:
  ...
  x0::TokenShaperError createBucket(const std::string& name, float rate,
                                    float ceil);
  ...
  Backend* createBackend(const std::string& name, const std::string& protocol,
                         const x0::SocketSpec& spec, size_t capacity,
                         BackendRole role);
 ...
 private:
  ...
  void serviceUnavailable(
      RequestNotes* notes,
      x0::HttpStatus status = x0::HttpStatus::ServiceUnavailable);
};
