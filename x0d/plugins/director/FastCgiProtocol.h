/* <plugins/director/FastCgiProtocol.h>
 *
...
public:
	Record(Type type, uint16_t requestId, uint16_t contentLength, uint8_t paddingLength) :
		version_(1),
...
public:
	...
	virtual void onParam(const char *name, size_t nameLen, const char *value, size_t valueLen) = 0;
};
...
public:
	EndRequestRecord(uint16_t requestId, uint32_t appStatus, ProtocolStatus protocolStatus) :
...
inline void CgiParamStreamReader::processParams(const char *buf, size_t length)
{
	const char *i = buf;
	size_t pos = 0;
