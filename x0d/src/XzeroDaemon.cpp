...
void XzeroDaemon::daemonize() {
  if (::daemon(true /*no chdir*/, true /*no close*/) < 0) {
    throw std::runtime_error(x0::fstringbuilder::format(
...
bool XzeroDaemon::verifyEnv() {
  ...
  if (stat(X0_TMPDIR, &st) < 0) {
    log(Severity::error, "Could not stat temp dir %s. %s", X0_TMPDIR,
        strerror(errno));
    return false;
  }
  ...
  if (access(X0_TMPDIR, W_OK) < 0) {
    log(Severity::error, "Could no access temp dir %s. %s", X0_TMPDIR,
        strerror(errno));
    return false;
  }
...
bool XzeroDaemon::setupConfig() {
  ...
  auto tokens =
      x0::Tokenizer<x0::BufferRef, x0::Buffer>::tokenize(instant_, ",");
  ...
  if (documentRoot_.empty())
    ...
    if (!realpath(documentRoot_.c_str(), result)) {
      log(x0::Severity::error, "Could not resolv document root: '%s'. %s",
...
bool XzeroDaemon::createPidFile() {
  ...
  if (pidfile_.empty()) {
    if (nofork_) {
      ...
      log(x0::Severity::error, "No PID file specified. Use %s --pid-file=PATH.",
          argv_[0]);
      return false;
    }
  }
  ...
  if (!pidfile) {
    log(x0::Severity::error, "Could not create PID file %s: %s.",
  ...
  log(x0::Severity::info, "Created PID file with value %d [%s]", getpid(),
      pidfile_.c_str());
...
void XzeroDaemon::reexec() {
  ...
  server_->log(x0::Severity::debug, "Setting envvar XZERO_LISTEN_FDS to: '%s'",
