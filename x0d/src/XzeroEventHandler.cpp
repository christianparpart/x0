...
x0::HttpServer* XzeroEventHandler::server() const { return daemon_->server(); }
...
void XzeroEventHandler::resumeHandler(ev::sig& sig, int) {
  server()->log(x0::Severity::debug, "Siganl %s received.",
...
void XzeroEventHandler::gracefulShutdownHandler(ev::sig& sig, int) {
  server()->log(x0::Severity::info, "%s received. Shutting down gracefully.",
                strsignal(sig.signum));

  for (x0::ServerSocket* listener : server()->listeners()) listener->close();
...
void XzeroEventHandler::quickShutdownHandler(ev::sig& sig, int) {
  daemon_->log(x0::Severity::info, "%s received. shutting down NOW.",
               strsignal(sig.signum));
