...
template <typename T>
List<T>::List()
    : head_(new Node(), 1), tail_(new Node(), 1), size_(0) {
  head_->next = tail_;
}
...
bool List<T>::remove(const T& value) {
  while (true) {
    ...
    if (unlikely(curr->value != value)) return false;

    auto succ = curr->next;

    //! attempt to mark \c curr as deleted by decrementing \c curr.next's tag by
    //one.
    if (!curr->next.tryTag(succ, succ.tag() - 1)) continue;

    if (succ.tag() - 1 == 0) delete curr.get();
...
T& List<T>::front() {
  ...
  retry:
    ...
    while (succ.tag() == 0) {
      if (pred->next.compareAndSwap(curr,
                                    TaggedPtr<Node>(succ.ptr(), succ.tag())))
        goto retry;
    ...
    if (curr != tail_) return curr->value;
...
typename List<T>::Window List<T>::find(const T& value) {
  ...
  retry:
    ...
    while (true) {
      auto succ = curr->next;
      while (succ.tag() == 0) {
        if (!pred->next.compareAndSwap(curr, succ)) goto retry;
