...
X0_API InstructionSig operandSignature(Opcode opc);
X0_API const char* mnemonic(Opcode opc);
X0_API size_t computeRegisterCount(const Instruction* code, size_t size);
X0_API size_t registerMax(Instruction instr);
X0_API FlowType resultType(Opcode opc);
