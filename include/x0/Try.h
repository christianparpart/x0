...
/**
 * Provides a typed value wrapper which also encodes a possible error status that avoids exception handling.
 */
template<typename T>
class X0_API Try {
public:
    ...
    const char* errorMessage() const { requireError(); return errorMessage_; }

    T& get() { require(); return value_; }
    const T& get() const { require(); return value_; }

    T& operator*() { require(); return value_; }
...
template<typename T> bool operator==(const Try<T>& a, const Try<T>& b) {
    if (a.isError() && b.isError())
        return strcmp(a.errorMessage(), b.errorMessage()) == 0;

    if (a.isOkay() && b.isOkay())
        return a.get() == b.get();
