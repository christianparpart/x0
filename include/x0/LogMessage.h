...
class X0_API LogMessage {
 public:
  ...
  BufferRef tagAt(size_t i) const {
    return tagBuffer_.ref(tags_[i].first, tags_[i].second);
  }
...
inline LogMessage::LogMessage(Severity severity, const char* msg)
    : severity_(severity), tagBuffer_(), messageSize_(), tags_() {
  tagBuffer_.push_back(msg);
  messageSize_ = tagBuffer_.size();
}

template <typename Arg1, typename... Args>
inline LogMessage::LogMessage(Severity severity, const char* msg, Arg1 arg1,
                              Args... args)
    : severity_(severity), tagBuffer_(), messageSize_(), tags_() {
