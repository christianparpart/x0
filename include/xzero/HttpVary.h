...
class XZERO_API HttpVary {
 public:
  enum class Match { None, ValuesDiffer, Equals, };
  ...
  Match match(const HttpRequest* r) const;
  Match match(const HttpVary& other) const;

  /**
   * Creates a HttpVary object, based on the Response request header
   *
   * @param vary HttpVary HTTP response header, a comma seperated list of
   *request headers.
  ...
  static std::unique_ptr<HttpVary> create(
      const U& vary, const std::vector<HttpHeader<T>>& requestHeaders);

  /**
   * Creates a HttpVary object, based on the Response request header
   */
  static std::unique_ptr<HttpVary> create(const HttpRequest* r);

  class iterator {
   public:
    iterator() {}
    iterator(HttpVary* vary, size_t i) : vary_(vary), i_(i), e_(vary->size()) {}
 ...
 private:
  template <typename T, typename U>
  static inline T find(const U& name,
                       const std::vector<HttpHeader<T>>& requestHeaders);
...
inline size_t HttpVary::size() const {
  return names_.size();  // this is okay since names.size() always equals
                         // values.size()
}

inline const std::vector<BufferRef>& HttpVary::names() const { return names_; }
...
template <typename T, typename U>
inline T HttpVary::find(const U& name,
                        const std::vector<HttpHeader<T>>& requestHeaders) {
  for (auto& i : requestHeaders)
    if (iequals(name, i.name)) return i.value;

  return T();
}

template <typename T, typename U>
std::unique_ptr<HttpVary> HttpVary::create(
    const U& varyHeader, const std::vector<HttpHeader<T>>& requestHeaders) {
  //	if (varyHeader.empty())
