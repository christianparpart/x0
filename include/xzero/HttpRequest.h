...
class XZERO_API HttpRequest :
  ...
  class HeaderList  // {{{
   ...
   public:
    struct Header {  // {{{
      ...
      Header(const std::string& _name, const std::string& _value, Header* _prev,
             Header* _next)
          : name(_name), value(_value), prev(_prev), next(_next) {
        link(prev, next);
      }

      Header(const BufferRef& _name, const BufferRef& _value, Header* _prev,
             Header* _next)
          : name(_name.str()), value(_value.str()), prev(_prev), next(_next) {
        link(prev, next);
      }

      Header(const char* _name, const char* _value, Header* _prev,
             Header* _next)
          : name(_name), value(_value), prev(_prev), next(_next) {
        link(prev, next);
      }
    ...
    class iterator {  // {{{
     ...
     public:
      ...
      explicit iterator(Header* item) : current_(item) {}

      Header& operator*() { return *current_; }

      Header& operator->() { return *current_; }

      Header* get() const { return current_; }
   ...
   public:
    HeaderList() : size_(0), first_(NULL), last_(NULL) {}
    ...
    Header* findHeader(const std::string& name) {
      ...
      while (item != NULL) {
        if (iequals(item->name.c_str(), name.c_str())) return item;
     ...
     *
     * If a header with the given name does exist, the new value and a comma
     *will be
    ...
    Header* unlinkHeader(Header* item) {
      ...
      if (item == first_) first_ = first_->next;

      if (item == last_) last_ = last_->prev;
 ...
 private:
  /// pre-computed string representations of status codes, ready to be used by
  /// serializer
 ...
 public:
  ...
  HttpConnection& connection;  ///< the TCP/IP connection this request has been
                               ///sent through
  ...
  HttpFileRef fileinfo;   ///< the final entity to be served, for example the
                          ///full path to the file on disk.
  int httpVersionMajor;  ///< HTTP protocol version major part that this request
                         ///was formed in
  int httpVersionMinor;  ///< HTTP protocol version minor part that this request
                         ///was formed in
  ...
  //request header value, if specified.
  //	std::shared_ptr<range_def> range;			//!< parsed "Range" request
  //header
  bool expectingContinue;

 public:
  void setErrorHandler(const std::function<bool(HttpRequest*)>& cb) {
    errorHandler_ = cb;
  }

  template <typename T, void (T::*cb)(Buffer& output)>
  void registerInspectHandler(T* object);
  ...
  bool isFinished() const {
    return connection.state() == HttpConnection::SendingReplyDone;
  }

  /**
   * Abnormally aborts given request and immediately closes the underlying
   * connection.
   */
...
inline bool HttpRequest::supportsProtocol(int major, int minor) const {
  if (major == httpVersionMajor && minor <= httpVersionMinor) return true;

  if (major < httpVersionMajor) return true;
...
bool HttpRequest::writeCallback(K* object) {
  return writeCallback([=]() { (object->*cb)(); });
}
 ...
 * \endcode
 *
 * \see BufferSource, CallbackSource, CompositeSource, FileSource, FilterSource,
 *Source
 */
template <class T, class... Args>
...
/*! retrieves the number of bytes successfully transmitted within this very
 *request.
 *
 * \note this includes protocol-dependant bytes, too, such as transfer encoding
...
inline bool HttpRequest::isResponseContentForbidden() const {
  return content_forbidden(status);
}
