...
class BASE_API DebugLogger {
 public:
  ...
  /**
   * @param tag Classifies the debug message to group certain kind of events
   * together.
   * @param level Debug verbosity. The higher the number the more verbose the
   * message.
   * @param fmt the debug message format string (printf-style)
   */
  template <typename... Args>
  void log(const std::string& tag, int level, const char* fmt,
           const Args&... args) {
  ...
  template <typename... Args>
  void logUntagged(const std::string& tag, int level, const char* fmt,
                   const Args&... args) {
    if (auto instance = get(tag)) {
   ...
   *
   * @param tag the unique tag that classifies all debug messages being passed
   *through this instance.
   * @return a debug stream instance that is used for only these kind of debug
   *messages.
   */
  static Instance* get(const std::string& tag) { return get()[tag]; }
   ...
   *
   * @param yield a callback function that is invoked for each tag instance
