...
class X0_API Process {
 ...
 public:
   ...
   *
   * \see start(const std::string& exe, const params& args, const environment&
   *env, const std::string& workdir)
   */
  explicit Process(struct ev_loop* loop);

  /** initializes this process object and actually starts a child program as
   *specified.
   *
   * \param io the io_service to use.
   ...
   * \param workdir optional workdir to change to before the program inside the
   *child process gets executed.
   *
   * \note you may only run one child at a time per process object.
   */
  Process(struct ev_loop* loop, const std::string& exe,
          const ArgumentList& args, const Environment& env = Environment(),
  ...
  /** retrieves the return status code of the child program, if exited, an
 ...
 private:
  /** setup routine to be invoked from within the child process, to setup the
   * child environment and exec'uting the child program. */
  void setupChild(const std::string& exe, const ArgumentList& args,
                  const Environment& env, const std::string& workdir);

  /** setup routine to be invoked right after the fork() within the parent
   * process. */
  void setupParent();
 ...
 private:
  ...
  mutable int status_;  //!< holds the child's process status, see system's
                        //waitpid() for more info.
};

// {{{ inlines
inline int Process::id() const { return pid_; }

inline int Process::input() { return input_.local(); }

inline void Process::closeInput() { input_.closeLocal(); }

inline int Process::output() { return output_.local(); }

inline int Process::error() { return error_.local(); }
// }}}
