...
Fast process shutdown (without waiting for the current requests to complete).
...
Reloading configuration file by starting a new process and gracefully shutting down the current. (TODO)
...
Upgrade executable on the fly. (TODO)
