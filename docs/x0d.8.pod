...
x0d [B<-c>|B<--config> I<PATH>] [B<-X>|B<--no-fork>] [B<-G>|B<--guard>] [B<--systemd>] \
    [B<-G>|B<--guard>] [B<-p>|B<--pid-file> I<PATH>] \
    [B<-u>|B<--user> I<USER>] [B<-g>|B<--group> I<GROUP>] \
    [B<-L>|B<--log-severity> I<error>|I<warning>|I<notice>|I<info>|I<diag>|I<debug>] \
    [B<-l>|B<--log-target> I<syslog>|I<file>|I<systemd>|I<console>] \
    [B<-l>|B<--log-file> I<PATH>] \
    [B<-i>|B<--instant> [I<PATH>,]I<PORT>]] \
    [B<-k>|B<--crash-handler>] \
...
B<x0d> will default to the current working directory, if not specified.
