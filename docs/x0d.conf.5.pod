...
Example 2:
 ...
 handler setup {
    ...
    listen 'fe80::2c0:26ff:fe85:b3:8080';
...
Example:
 ...
 handler custom_error {
     if req.path =^ '/static' {
         ...
         fastcgi 'unix:/var/run/custom-error-generator.socket'
     }
 }
...
=item B<fastcgi> (unix:path)

Passes the current request to a FastCGI backend application, listening
on a UNIX domain socket.

Example:

 fastcgi 'unix:/var/run/fcgiapp.socket';
...
=item B<proxy.reverse> (hostname:port)
...
Defines a colon-seperated list of C++ classes to print debugging information for.
