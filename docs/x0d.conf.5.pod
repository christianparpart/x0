...
=item B<workers.affinity> (array_of_numbers)
...
That is, when you use C<workers.affinity [0, 1, 4];> it will spawn three workers
...
=item B<director.fcgi> (address: IP, port: number)

Directly proxies a givenr equest to FastCGI backend application.

=item B<director.http> (address: IP, port: number)
