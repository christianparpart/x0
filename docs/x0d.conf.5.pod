...
=head1 SYNOPSIS
 ...
 handler setup {
     ...
     listen 'bind' => 0.0.0.0, 'port' => 8080, 'backlock' => 128;
 }
...
The configuration-engine behind B<x0d> is B<Flow>, a sub-project of B<x0>.
...
Example:
 ...
 handler custom_error {
     if req.path =^ '/static' {
         ...
         fastcgi 'path' => '/var/run/custom-error-generator.socket'
     }
 }
...
An example use looks as follows:
 ...
 fastcgi 'bind' => 127.0.0.1, 'port' => 1234;

or, for listening on a UNIX domain socket:

 fastcgi 'path' => '/var/run/program.fcgi';
...
=item B<proxy.cloak> (flag)

Setup property, defining whether or not to cloak the server tag.

=item B<proxy.reverse> (backend...)

Handler, forwarding the request to a backend HTTP server.

An example use looks as follows:

 proxy.reverse 'bind' => 127.0.0.1, 'port' => 1234;

or, for listening on a UNIX domain socket:

 proxy.reverse 'path' => '/var/run/program.fcgi';
...
L<x0d(8)>, L<spawn-fcgi(1)>

L<http://redmine.trapni.de/projects/xzero>, x0 project management page
