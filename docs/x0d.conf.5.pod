...
Load balances incoming requests to an upstream cluster whose name
matches the requests Host headers hostname.
...
=item B<proxy.http> (address: ip, port: int, on_client_abort: string)
