...
=item B<listen> (address: IP, port: number, backlog: number = 128, reuse_addr: bool = false, multi_accept: number = 1)
...
Example:
 ...
 handler custom_error {
     if req.path =^ '/static' {
         ...
         director.fcgi address: 127.0.0.1, port: 7500;
     }
 }
...
Example:
 ...
 handler setup {
   accesslog.format 'custom', "%c|%t|%T|%r|%h|%>{User-Agent}";
 }
...
=item B<accesslog.sysloc> (string format = "main")
...
=item B<director.load> (name: string, path: string)
...
=item B<director.balance> (director_name: string, bucket_name: string = "")

Passes the request to the director by given name.
This is usually what you are to use in your configuration file.

You can optionally pass a bucket name if you partitioned your cluster into buckets.

Example:

 handler main {
     if (req.path =^ '/cart') {
         director.pass 'app_cluster', 'vip';
     } else {
         director.pass 'app_cluster';
     }
 }
...
=item B<director.fcgi> (address: IP, port: number, string: on_client_abort = "close")
...
=item B<director.http> (address: IP, port: number, string: on_client_abort = "close")

Directly proxies a givenr equest to an HTTP backend application.
...
=item B<director.cache> (enabled: bool = true)
...
Use this in order improve user experience by accelerating your backends via this central cache.
 ...
 handler myapp {
   ...
   docroot '/srv/myapp/public';
   precompressed;
   staticfile;

   # Pass request to the 'myapp'-cluster.
   director.balance 'myapp';
 }
...
=item B<ssl.listen> (bind: IP, port: int, backlog: int = 128, multi_accept: int = 1, reuse_port: bool = true)
...
B<x0d> (daemon, core framework, and plugins) is written by Christian Parpart <trapni@gmail.com>
