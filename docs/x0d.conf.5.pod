...
=item B<workers> (I<array_of_numbers>: int[])
...
=item B<request_header_buffer_size> (I<size>: int)
...
=item B<request_body_buffer_size> (I<size>: int)
...
=item B<listen> (I<address>: IP = 0.0.0.0, I<port>: int, I<backlog>: int = 128, I<multi_accept>: int = 1, I<reuse_addr>: bool = false, I<reuse_port> = false)
...
=item B<error.page> (I<status>: int, I<uri>: string)
...
=item B<sys.domainname>

Retrieves the hosts domain name.

Example:

 log.info("domainname: " + sys.domainname);

=item B<sys.max_conn>

Retrieves the total number of connections the server can accept simultanously.
See listen(2)'s backlog parameter.

On Linux systems, this value can be tweaked in I</proc/sys/net/core/somaxconn>.

Example:

 setup {
   listen port: 80, backlog: sys.max_conn;
 }

=item B<sleep> (I<seconds>: Duration)
...
=item B<file.exists> (I<path>: string)
...
=item B<file.is_reg> (I<path>: string)
...
=item B<file.is_dir> (I<path>: string)
...
=item B<file.is_exe> (I<path>: string)
...
=item B<req.accept_language> (I<supportedLanguages>: string[])
...
=item B<req.header> (I<name>: string)
...
=item B<req.cookie> (I<name>: string)
...
=item B<regex.group> (I<position>: int)
...
=item B<autoindex> (I<filenames>: string[])
...
=item B<alias> (I<prefix>: string, I<target>: string)
...
=item B<header.add> (I<name>: string, I<value>: string)

Adds the given header to the list of response headers.

=item B<header.overwrite> (I<name>: string, I<value>: string)

Adds (possibly overwriting if already present) a response header.

=item B<header.append> (I<name>: string, I<value>: string)
...
=item B<header.remove> (I<name>: string)
...
=item B<return> (I<code>: int, I<url>: string)
...
=item B<accesslog.console> (string format = "main")

is a function to ensure the current request is being logged
onto B<x0d>'s console (stdout).
...
=item B<proxy.cluster> (name: string, path: string = @CLUSTERDIR@/NAME.cluster.conf, bucket: string = "", backend: string = "")
...
=item B<proxy.http> (I<address>: IP, I<port>: int, I<on_client_abort>: string = "close", I<connect_timeout>: Duration = 10s, I<read_timeout>: Duration = 60s, I<write_timeout>: Duration = 10s)
