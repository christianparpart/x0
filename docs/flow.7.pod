...
Matches are a very powerful new feature in B<Flow>. The allow you to effeciently match on multiple possible cases,
and let each distinct case run its own code blocks.

Also, using the match statement makes your source file look clean, easy to read and maintain.

Supported match operations are:
