 ...
 *
 * setup API:
 *     int cgi.ttl = 5;                    ; max time in seconds a cgi may run
 *until SIGTERM is issued (0 for unlimited).
 *     int cgi.kill_ttl = 5                ; max time to wait from SIGTERM on
 *before a SIGKILL is ussued (0 for unlimited).
 *     int cgi.max_scripts = 20            ; max number of scripts to run in
 *concurrently (0 for unlimited)
 *
 * request processing API:
 *     handler cgi.exec()                  ; processes executable files as CGI
 *(apache-style: ExecCGI-option)
 *     handler cgi.run(string executable)  ; processes given executable as CGI
 *on current requested file
 *
 * notes:
 ...
 * - allow keep-alive on fast closing childs by adding content-length manually
 *(if not set already)
 * - pass child's stderr to some proper log stream destination
 * - close child's stdout when client connection dies away before child process
 *terminated.
 *   - this implies, that we should still watch on the child process to
 *terminate
 ...
 *      cgi.ttl(TimeSpan::fromSeconds(60));     // define maximum ttl this
 *script may run
 * 		cgi.runAsync();
 * 	}
 * \endcode
 */
class CgiScript : public HttpMessageParser {
 public:
 ...
 private:
  ...
  unsigned long long serial_;  //!< used to detect wether the cgi process
                               //actually generated a response or not.
...
CgiScript::CgiScript(HttpRequest* r, const std::string& hostprogram)
  ...
  request_->setAbortHandler([this]() { process_.terminate(); });
}
...
void CgiScript::onCheckDestroy(ev::async& /*w*/, int /*revents*/) {
  // libev invoked waitpid() for us, so re-use its results by passing it to
  // Process
  ...
  if (process_.expired()) {
    // process exited; do not wait for any child I/O stream to complete, just
    // kill us.
...
bool CgiScript::checkDestroy() {
  ...
  std::string fs;
  if (outputFlags_ & StdoutClosed) fs = "|stdout";
  if (outputFlags_ & StderrClosed) fs += "|stderr";
...
inline void _loadenv_if(const std::string& name,
...
inline void CgiScript::runAsync() {
  ...
  environment["SERVER_PORT"] =
      std::to_string(request_->connection.localPort());  // TODO this should to
                                                         // be itoa'd only ONCE

  environment["REQUEST_METHOD"] = request_->method.str();
  environment["REDIRECT_STATUS"] =
      "200";  // for PHP configured with --force-redirect (Gentoo/Linux e.g.)

  request_->updatePathInfo();
  environment["PATH_INFO"] = request_->pathinfo.str();
  if (!request_->pathinfo.empty()) {
    environment["PATH_TRANSLATED"] =
        request_->documentRoot + request_->pathinfo.str();
    environment["SCRIPT_NAME"] =
        request_->path.ref(0, request_->path.size() - request_->pathinfo.size())
            .str();
  } else {
  ...
  environment["REMOTE_PORT"] =
      std::to_string(request_->connection.remotePort());
  ...
  if (request_->contentAvailable()) {
    ...
    environment["CONTENT_LENGTH"] =
        request_->requestHeader("Content-Length").str();
  } else {
    process_.closeInput();
  }
  ...
  for (const auto& header : request_->requestHeaders) {
    ...
    for (auto ch : header.name) {
      key.push_back(
          static_cast<char>(std::isalnum(ch) ? std::toupper(ch) : '_'));
    }
...
// }}}
  ...
  // redirect process_'s stdout/stderr to own member functions to handle its
  // response
  ...
  // libev's ev++ (at least till version 3.80) does not initialize `sent` to
  // zero)
      ...
      switch (errno) {
        ...
        default:
          request_->log(Severity::error,
                        "Writing request body to CGI failed. %s",
                        strerror(errno));
          evStdin_.stop();
          process_.closeInput();
          return;
...
void CgiScript::onStdoutAvailable(ev::io& w, int revents) {
  ...
  int rv = ::read(process_.output(), outbuf_.end(),
  ...
  if (rv > 0) {
    ...
    if (rv != EINTR && rv != EAGAIN) {
      ...
      request_->log(Severity::error,
                    ...
                    request_->fileinfo->path().c_str(), strerror(errno));

      if (!serial_) {
        request_->status = HttpStatus::InternalServerError;
        request_->log(Severity::error, "CGI script generated no response: %s",
...
/** consumes any output read from the CGI's stderr pipe and either logs it into
 ...
 */
class CgiPlugin : public x0d::XzeroPlugin {
 private:
 ...
 public:
  CgiPlugin(x0d::XzeroDaemon* d, const std::string& name)
      : x0d::XzeroPlugin(d, name), ttl_(0), maxScripts_(128) {
    setupFunction("cgi.ttl", &CgiPlugin::set_ttl, FlowType::Number);
    setupFunction("cgi.max_scripts", &CgiPlugin::setMaxScripts,
                  FlowType::Number);

    mainHandler("cgi.exec", &CgiPlugin::exec);
    mainHandler("cgi.run", &CgiPlugin::run, FlowType::String);

    // TODO: DAG walk to "verify" that docroot was set by known core flow
    // functions (docroot)
  }
...
X0D_EXPORT_PLUGIN_CLASS(CgiPlugin)
