...
void SslContext::setLogger(Logger *logger) { logger_ = logger; }
...
void SslContext::setCertFile(const std::string &filename) {
  ...
  if (error_) {
    log(Severity::error, "Error loading certificate file(%s): %s",
...
void SslContext::setKeyFile(const std::string &filename) {
  ...
  if ((error_ = loadFile(data, filename))) {
    log(Severity::error, "Error loading private key file(%s): %s",
        filename.c_str(), error_.message().c_str());
    return;
  }
