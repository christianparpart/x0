 ...
 *
 * These images can either be located directly on local disk or passed as a
 *remote URL to fetch.
 *
 * Feature Goals:
 * <ul>
 ...
 *   <li>allow fetching source image from local disk as well as remote HTTP URLs
 *(GET request)
 *   <li>security: query arg "key" to be xor'd against a PSK that must result
 *into a timestamp that may not differ more than N seconds to be accepted.
 *   <li>image processing outside http workers in dedicated worker cluster.
 * </ul>
 *
 * Flow API:
 * <code>
 *   // number of processing worker threads, just like `workers` but for image
 *processing
 ...
 *   property string imageable.tempdir = '/var/tmp/base/imageable';
 *
 ...
 *
 * Example URLs:
 * - http://i.dawanda.com/fit/7f848c35/Vorsaetze-fuer-2013.jpg  ?crop=true
 *&size=160x120
 *&url=http%3A%2F%2Fs32.dawandastatic.com%2FCampaignImage%2F193%2F193734%2F1356687119-440.jpeg
 * - http://i.dawanda.com/fit/8809d1bf/Du-schaffst-es.jpg       ?crop=true
 *&size=336x252
 *&url=http%3A%2F%2Fs31.dawandastatic.com%2FCampaignImage%2F190%2F190098%2F1354885195-841.jpeg
 * - http://i.dawanda.com/crop/9b9a3da6/Vorsaetze-fuer-2013.jpg
 *?crop=589x442%2B9%2B140 &size=336x252
 *&url=http%3A%2F%2Fs32.dawandastatic.com%2FCampaignImage%2F193%2F193686%2F1356685759-501.jpeg
 */
...
// {{{ UrlFetcher API
class UrlFetcher : public HttpMessageParser {
 public:
  static UrlFetcher* fetch(HttpWorker* worker, const Url& url,
                           const std::function<void()>& completionHandler);

  UrlFetcher(HttpWorker* worker, const Url& url,
             const std::function<void()>& completionHandler);
...
class ImageableProcessor : public Actor<Imageable*> {
 protected:
...
Imageable::Imageable(HttpRequest* r) : request_(r), wand_(NewMagickWand()) {}

Imageable::~Imageable() { DestroyMagickWand(wand_); }
...
void Imageable::processImage() {
  MagickBooleanType status =
      MagickReadImage(wand_, request_->fileinfo->path().c_str());
  ...
  // TRACE(1, "width:%d, height:%d, iters:%d, x:%.2f, y:%.2f", width, height,
  // iterations, x, y);
  ...
  request_->responseHeaders.push_back("Content-Length",
  ...
  if (fd < 0) {
    request_->log(Severity::error, "Could not open file: '%s': %s",
                  fileinfo->filename().c_str(), strerror(errno));
    request_->status = HttpStatus::InternalServerError;
    request_->finish();
    delete this;
    return;
...
void ImageableProcessor::process(Imageable* imageable) {
  // perform the actual resize-fit-crop action, then pass response transfer job
  // back to http worker.
  imageable->perform();
}
// }}}
// {{{ ImageablePlugin
class ImageablePlugin : public XzeroPlugin {
 private:
  ImageableProcessor* processor_;

 public:
  ImageablePlugin(HttpServer& srv, const std::string& name)
      : XzeroPlugin(srv, name), processor_(nullptr) {
    setupFunction("imageable.workers", &ImageablePlugin::setWorkers,
                  FlowType::Number);
  ...
  ~ImageablePlugin() { MagickWandTerminus(); }
...
X0D_EXPORT_PLUGIN_CLASS(ImageablePlugin)
// }}}
