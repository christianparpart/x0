...
SocketSpec SocketSpec::fromString(const std::string& value) {
  ...
  if (slen == 0) return SocketSpec();

  if (value.find("unix:") == 0) return SocketSpec::fromLocal(value.substr(5));

  if (value[0] == '[') {  // IPv6
    auto e = value.find("]");
    if (e == std::string::npos) return SocketSpec();

    if (e + 1 <= slen) return SocketSpec();

    if (value[e + 1] != ':') return SocketSpec();
...
SocketSpec SocketSpec::fromInet(const IPAddress& ipaddr, int port,
                                int backlog) {
  return SocketSpec(ipaddr, port, backlog);
}

void SocketSpec::setPort(int value) { port_ = value; }

void SocketSpec::setBacklog(int value) { backlog_ = value; }

void SocketSpec::setMultiAcceptCount(size_t value) {
  multiAcceptCount_ = value;
}

void SocketSpec::setReusePort(bool value) { reusePort_ = value; }
