...
Socket::~Socket() {
  TRACE(1, "destroying. fd:%d, local(%s:%d)", fd_, localIP().c_str(),
        localPort());
...
static inline bool applyFlags(int fd, int flags) {
  return flags ? fcntl(fd, F_SETFL, fcntl(fd, F_GETFL) | flags) == 0 : true;
}
...
Socket* Socket::open(struct ev_loop* loop, const SocketSpec& spec, int flags) {
  ...
  if (state == Connecting) socket->setMode(Write);

  return socket;
}

Socket::State Socket::open(struct ev_loop* loop, const SocketSpec& spec,
                           int flags, int* fd) {
  // compute type-mask
  int typeMask = 0;
  ...
  if (spec.isLocal()) {
    *fd = ::socket(PF_UNIX, SOCK_STREAM | typeMask, 0);
    ...
    if (!applyFlags(*fd, flags)) return Closed;

    struct sockaddr_un addr;
    addr.sun_family = AF_UNIX;
    size_t addrlen = sizeof(addr.sun_family) +
                     strlen(strncpy(addr.sun_path, spec.local().c_str(),
    ...
    *fd = ::socket(spec.ipaddr().family(), SOCK_STREAM | typeMask, IPPROTO_TCP);
    ...
    switch (spec.ipaddr().family()) {
      case IPAddress::V4:
        ...
        memcpy(&((sockaddr_in*)buf)->sin_addr, spec.ipaddr().data(),
               spec.ipaddr().size());
        break;
      case IPAddress::V6:
        ...
        memcpy(&((sockaddr_in6*)buf)->sin6_addr, spec.ipaddr().data(),
               spec.ipaddr().size());
        break;
      default:
        ::close(*fd);
...
void Socket::close() {
  TRACE(1, "close: fd=%d (state:%s, timer_active:%s)", fd_, state_str(),
        timer_.is_active() ? "yes" : "no");
...
ssize_t Socket::read(Buffer& result) {
  ...
  for (;;) {
    if (result.capacity() - result.size() < 256) {
      result.reserve(std::max(static_cast<std::size_t>(4096),
                              static_cast<std::size_t>(result.size() * 1.5)));
    }
...
ssize_t Socket::read(Pipe* pipe, size_t size) {
  if (splicing()) {
    ...
    if (errno == ENOSYS) setSplicing(false);
...
ssize_t Socket::write(int fd, off_t* offset, size_t nbytes) {
  ...
  if (rv < 0 && errno != EINTR && errno != EAGAIN)
    ERROR("Socket(%d).write(): sendfile: rv=%ld (%s)", fd_, rv,
          strerror(errno));
#else
  (void)offset0;
#endif
...
void Socket::timeout(ev::timer& timer, int revents) {
  ...
  if (timeoutCallback_) timeoutCallback_(this, timeoutData_);
}
...
std::string Socket::remote() const {
  ...
  switch (addressFamily_) {
    case AF_INET:
      ...
      break;
    case AF_INET6:
      n = snprintf(buf, sizeof(buf), "[%s]:%d", remoteIP().c_str(),
                   remotePort());
      break;
    default:
