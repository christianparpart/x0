...
static std::string& gsub(std::string& buf, const std::string& src,
...
XzeroDaemon::~XzeroDaemon() {
  ...
  while (!plugins_.empty()) unloadPlugin(plugins_[plugins_.size() - 1]->name());
...
int XzeroDaemon::run() {
  ...
  HttpConnection::setTempDirectory(getenv("TMPDIR"));
...
#if defined(HAVE_SYSLOG_H)
  SyslogSink::open("x0d", LOG_PID | LOG_NDELAY, LOG_DAEMON);
#endif

  if (!parseCommandLineArgs()) return 1;

  eventHandler_ = new XzeroEventHandler(this, ev::default_loop(evFlags_));
  server_ = new HttpServer(eventHandler_->loop(), generation);
  ...
  if (systemd_) {
    nofork_ = true;
    server_->setLogger(std::make_shared<SystemdLogger>());
  } else {
    if (logTarget_ == "file") {
      if (!logFile_.empty()) {
        auto logger = std::make_shared<FileLogger>(logFile_, [this]() {
        ...
        server_->setLogger(std::make_shared<SystemLogger>());
      }
    } else if (logTarget_ == "console") {
      server_->setLogger(std::make_shared<ConsoleLogger>());
  ...
  if (!setupConfig()) {
    log(Severity::error, "Could not start x0d.");
    return -1;
  }

  if (!drop_privileges(user_, group_)) return -1;
  ...
  if (!nofork_) daemonize();

  if (!createPidFile()) return -1;

  if (group_.empty()) group_ = user_;
  ...
  if (!systemd_ && !pidfile_.empty()) unlink(pidfile_.c_str());
...
bool XzeroDaemon::parseCommandLineArgs() {
  ...
  static const char* package_header =
      "x0d: Xzero HTTP Web Server, version " PACKAGE_VERSION
      " [" PACKAGE_HOMEPAGE_URL "]";
  ...
  for (;;) {
    int long_index = 0;
    switch (getopt_long(argc_, argv_, "vyf:O:p:u:g:o:l:s:i:e:khXV",
      ...
      case 'o':
        ...
        if (logTarget_ != "file" && logTarget_ != "console" &&
            logTarget_ != "syslog" && logTarget_ != "systemd") {
      ...
      case 'i':
        ...
        break;
      case 'v':
        std::cout << package_header << std::endl << package_copyright
                  << std::endl;
      case 'y':
      ...
      case 'h':
        std::cout << package_header << std::endl << package_copyright
                  << std::endl << std::endl << package_license << std::endl
                  << std::endl << "usage:" << std::endl << "  x0d [options ...]"
                  << std::endl << std::endl << "options:" << std::endl
                  << "  -h,--help                 print this help" << std::endl
                  << "  -f,--config=PATH          specify a custom "
                     "configuration file [" << configfile_ << "]" << std::endl
                  << "  -O,--optimization-level=N sets the configuration "
                     "optimization level [" << optimizationLevel_ << "]"
                  << std::endl
                  << "  -X,--no-fork              do not fork into background"
                  << std::endl
                  << "     --systemd              force systemd-mode, which is "
                     "auto-detected otherwise" << std::endl
                  << "  -p,--pid-file=PATH        PID file to create"
                  << std::endl
                  << "  -u,--user=NAME            user to drop privileges to"
                  << std::endl
                  << "  -g,--group=NAME           group to drop privileges to"
                  << std::endl << "  -o,--log-target=TARGET    log target, one "
                                  "of: file, console, syslog, systemd [file]"
                  << std::endl << "  -l,--log-file=PATH        path to log "
                                  "file (ignored when log-target is not file)"
                  << std::endl
                  << "  -s,--log-severity=VALUE   log severity level, one of: "
                     "error, warning, notice, info, debug, debug1..6 ["
                  << logLevel_.c_str() << "]" << std::endl
                  << "  -i,--instant=PATH[,PORT]  run x0d in simple "
                     "pre-configured instant-mode,\n"
                  << "                            also implies --no-fork and "
                     "--log-target=console" << std::endl
                  << "  -k,--crash-handler        installs SIGSEGV crash "
                     "handler to print backtrace onto stderr." << std::endl
                  << "     --dump-ast             dumps parsed AST of the "
                     "configuration file (for debugging purposes)" << std::endl
                  << "     --dump-ir              dumps IR of the "
                     "configuration file (for debugging purposes)" << std::endl
                  << "     --dump-tc              dumps target code of the "
                     "configuration file (for debugging purposes)" << std::endl
                  << "  -v,--version              print software version"
                  << std::endl << "  -y,--copyright            print software "
                                  "copyright notice / license" << std::endl
                  << "     --splash               print splash greeter to "
                     "terminal on startup" << std::endl
                  << "  -V,--info                 print software version and "
                     "compile-time enabled core features" << std::endl
                  << std::endl;
        return false;
      case 'V': {
        std::cout << package_header << std::endl << package_copyright
      ...
      case -1: {  // EOF - everything parsed.
        if (optind == argc_) return true;

        Buffer args;
        while (optind < argc_) args.printf(" %s", argv_[optind++]);
...
void XzeroDaemon::daemonize() {
  if (::daemon(true /*no chdir*/, true /*no close*/) < 0) {
    throw std::runtime_error(fstringbuilder::format(
        "Could not daemonize process: %s", strerror(errno)));
  }
}

/** drops runtime privileges current process to given user's/group's name. */
bool XzeroDaemon::drop_privileges(const std::string& username,
                                  const std::string& groupname) {
  if (!groupname.empty() && !getgid()) {
    if (struct group* gr = getgrnam(groupname.c_str())) {
      if (setgid(gr->gr_gid) != 0) {
        log(Severity::error, "could not setgid to %s: %s", groupname.c_str(),
            strerror(errno));
        return false;
      }
  ...
  if (!username.empty() && !getuid()) {
    if (struct passwd* pw = getpwnam(username.c_str())) {
      if (setuid(pw->pw_uid) != 0) {
        log(Severity::error, "could not setgid to %s: %s", username.c_str(),
            strerror(errno));
        return false;
      }
      ...
      if (chdir(pw->pw_dir) < 0) {
        log(Severity::error, "could not chdir to %s: %s", pw->pw_dir,
            strerror(errno));
        return false;
      }
    } else {
...
bool XzeroDaemon::verifyEnv() {
  const char* tmpdir = xzero::HttpConnection::tempDirectory().c_str();
  struct stat st;

  if (stat(tmpdir, &st) < 0) {
    log(Severity::error, "Could not stat temp dir %s. %s", tmpdir,
        strerror(errno));
    return false;
  }

  if (!S_ISDIR(st.st_mode)) {
    log(Severity::error, "Temp dir \"%s\" must be a directory.", tmpdir);
    return false;
  }

  if (access(tmpdir, W_OK) < 0) {
    log(Severity::error, "Could no access temp dir %s. %s", tmpdir,
        strerror(errno));
    return false;
  }
...
bool XzeroDaemon::setupConfig() {
  if (instant_.empty()) {
    ...
    return setup(std::make_unique<std::ifstream>(configfile_), configfile_,
                 optimizationLevel_);
  }
  ...
  auto tokens =
      Tokenizer<BufferRef, Buffer>::tokenize(instant_, ",");
  ...
  if (documentRoot_.empty())
    ...
    if (!realpath(documentRoot_.c_str(), result)) {
      log(Severity::error, "Could not resolv document root: '%s'. %s",
          documentRoot_.c_str(), strerror(errno));
      return false;
    } else {
  ...
  return setup(std::make_unique<std::istringstream>(source),
               "instant-mode.conf", optimizationLevel_);
}
...
bool XzeroDaemon::createPidFile() {
  if (systemd_) {
    if (!pidfile_.empty())
      log(Severity::info,
          "PID file requested but process is being supervised by systemd. "
          "Ignoring.");
  ...
  if (pidfile_.empty()) {
    if (nofork_) {
      ...
      log(Severity::error, "No PID file specified. Use %s --pid-file=PATH.",
          argv_[0]);
      return false;
    }
  }
  ...
  if (!pidfile) {
    log(Severity::error, "Could not create PID file %s: %s.",
  ...
  log(Severity::info, "Created PID file with value %d [%s]", getpid(),
      pidfile_.c_str());

  return true;
}

/** starts new binary with (new) config - as child process, and gracefully
 * shutdown self.
 */
void XzeroDaemon::reexec() {
  if (eventHandler_->state() != XzeroState::Running) {
    server_->log(Severity::info, "Reexec requested again?. Ignoring.");
    return;
  }
  ...
  for (HttpWorker* worker : server_->workers()) {
    worker->suspend();
  }

  Buffer serializedListeners;

  for (ServerSocket* listener : server_->listeners()) {
    // stop accepting new connections
    listener->stop();

    // and clear O_CLOEXEC on listener socket, as we want to probably resume
    // these listeners in the child process
  ...
  if (systemd_) args.push_back("--systemd");
  ...
  switch (childPid) {
    case 0:
      ...
      server_->log(Severity::error,
                   "Executing new child process failed: %s", strerror(errno));
      abort();
      break;
    case -1:
      // error
      server_->log(Severity::error,
                   "Forking for new child process failed: %s", strerror(errno));
      break;
    default:
      // in parent
      // the child process must tell us whether to gracefully shutdown or to
      // resume.
      eventHandler_->setupChild(childPid);

      // we lost ownership of the PID file, if we had one as the child
      // overwrites it.
      pidfile_ = "";

      // FIXME do we want a reexecTimeout, to handle possible cases where the
      // child is not calling back? to kill them, if so!
      break;
  }
...
void XzeroDaemon::cycleLogs() {
  for (auto plugin : plugins_) plugin->cycleLogs();
...
void crashHandler(int nr, siginfo_t* info, void* ucp) {
  ...
  unsigned char* pc =
      reinterpret_cast<unsigned char*>(uc->uc_mcontext.gregs[REG_EIP]);
  fprintf(stderr, "Received SIGSEGV at %p.\n", pc);
#else
...
// }}}
// {{{ plugin management
std::string XzeroDaemon::pluginDirectory() const { return pluginDirectory_; }
 ...
 */
XzeroPlugin* XzeroDaemon::loadPlugin(const std::string& name,
                                     std::error_code& ec) {
  ...
  if (!pluginDirectory_.empty() &&
      pluginDirectory_[pluginDirectory_.size() - 1] != '/')
    pluginDirectory_ += "/";
  ...
  if (!ec) {
    plugin_create_t plugin_create =
        reinterpret_cast<plugin_create_t>(lib.resolve(plugin_create_name, ec));
...
std::vector<std::string> XzeroDaemon::pluginsLoaded() const {
  ...
  for (auto plugin : plugins_) result.push_back(plugin->name());
...
bool XzeroDaemon::import(const std::string& name, const std::string& path,
  ...
  if (ec) {
    log(Severity::error, "Error loading plugin: %s: %s", filename.c_str(),
        ec.message().c_str());
    return false;
  }
...
bool XzeroDaemon::setup(std::unique_ptr<std::istream>&& settings,
                        const std::string& filename, int optimizationLevel) {
  ...
  if (dumpAST_) ASTPrinter::print(unit_.get());
  ...
  if (dumpTargetCode_) program_->dump();
  ...
  for (auto i : plugins_)
    if (!i->post_config()) goto err;
  // }}}

  // {{{ run post-check hooks
  TRACE(1, "setup: post_check");
  for (auto i : plugins_)
    if (!i->post_check()) goto err;
  // }}}
  ...
  // some (configure-time related) events pending, i.e. director's (fcgi) health
  // checker
  // FIXME this is more a workaround than a fix.
  for (auto worker : server_->workers()) worker->wakeup();
...
bool XzeroDaemon::validateConfig() {
  ...
  if (!validate("setup", setupCalls.calls(), setupApi_)) return false;

  FlowCallVisitor mainCalls(mainFn);
  if (!validate("main", mainCalls.calls(), mainApi_)) return false;

  TRACE(1, "validateConfig finished");
  return true;
}

bool XzeroDaemon::validate(const std::string& context,
  ...
  for (const auto& i : calls) {
    ...
    if (std::find(api.begin(), api.end(), i->callee()->name()) == api.end()) {
      log(Severity::error,
          ...
          i->callee()->name().c_str(), context.c_str());
