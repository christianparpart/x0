...
class CgiParamStreamWriter {
 ...
 public:
  ...
  void encode(const xzero::BufferRef &name, const xzero::BufferRef &value) {
    encode(name.data(), name.size(), value.data(), value.size());
  }

  void encode(const std::string &name, const xzero::BufferRef &value) {
    encode(name.data(), name.size(), value.data(), value.size());
  }
  ...
  xzero::Buffer& output() { return buffer_; }
};
