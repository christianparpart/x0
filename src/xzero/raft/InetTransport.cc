...
Connection* InetTransport::create(InetConnector* connector,
                                  InetEndPoint* endpoint) {
...
RefPtr<InetEndPoint> InetTransport::getEndPoint(Id target) {
  {
    std::lock_guard<decltype(endpointLock_)> lk(endpointLock_);
    auto i = endpoints_.find(target);
    if (i != endpoints_.end()) {
      RefPtr<InetEndPoint> ep = i->second;
      endpoints_.erase(i);
  ...
  RefPtr<InetEndPoint> ep = endpointCreator_(*address);
  if (ep) {
...
void InetTransport::watchEndPoint(Id target, RefPtr<InetEndPoint> ep) {
  std::lock_guard<decltype(endpointLock_)> lk(endpointLock_);
...
void InetTransport::send(Id target, const VoteRequest& msg) {
  if (RefPtr<InetEndPoint> ep = getEndPoint(target)) {
    Buffer buffer;
