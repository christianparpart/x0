...
class XZERO_BASE_API Buffer : public MutableBuffer<mutableEnsure> {
 ...
 public:
  ...
  size_t mark() const XZERO_BASE_NOEXCEPT;
...
inline size_t Buffer::mark() const XZERO_BASE_NOEXCEPT {
  return mark_;
}
...
namespace xzero {
  ...
  template <typename T>
  uint32_t _hash(const T& array) XZERO_BASE_NOEXCEPT {
...
struct hash<xzero::BufferSlice> {
  ...
  result_type operator()(const argument_type& value) const XZERO_BASE_NOEXCEPT {
