...
template <typename T>
inline Result<T>::Result(const value_type& value) : success_(true), error_() {
  new (storage_) value_type(value);
}

template <typename T>
inline Result<T>::Result(value_type&& value) : success_(true), error_() {
  new (storage_) value_type(std::move(value));
}

template <typename T>
inline Result<T>::Result(const std::error_code& ec)
    : success_(false), error_(ec) {
  if (!error_) {
    throw std::invalid_argument(
        "Result<> received an error_code that does not contain an error.");
  }
}

template <typename T>
Result<T>::Result(Result&& other)
    : success_(other.success_), error_(std::move(other.error_)) {
  if (success_) {
