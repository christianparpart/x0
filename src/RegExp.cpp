...
RegExp::RegExp() : pattern_(), re_(NULL) {}
...
RegExp::~RegExp() { pcre_free(re_); }

RegExp::RegExp(RegExp&& v) : pattern_(std::move(v.pattern_)), re_(v.re_) {
  v.re_ = nullptr;
}
...
#ifndef NDEBUG
  for (size_t i = 0; i < OV_COUNT; ++i) ov[i] = 1337;
#endif
...
const char* RegExp::c_str() const { return pattern_.c_str(); }

RegExpContext::RegExpContext() : regexMatch_(nullptr) {}

RegExpContext::~RegExpContext() {
  if (regexMatch_ != nullptr) delete regexMatch_;
}
